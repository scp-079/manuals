...
- 使用两条竖线 `||` 分隔同义的关键词。
- 如需特别指定某个关键词以全匹配模式触发，可使用双花括号包裹关键词，例如： `{{关键词}}`，不过请注意，此用法在 `regex` 模式下无效。 
...
- 第一行，设置关键词的匹配模式，默认为 `include`，并且大小写不敏感，如需指定多个模式，请将模式代号以空格分隔，可选的模式有：
    ...
    - `forward`：仅匹配被转发的消息。
    - `pure`：去除名称中的干扰字符。
    - `regex`：使用正则匹配模式，每个以 `||` 分隔的关键词都被当做一条独立的正则表达式，正则表达式的语法请参照 Google RE2 的 [Wiki](https://github.com/google/re2/wiki/Syntax)。
